...
from src.rag_app.template import fill_template_for_all_files
...
def main() -> None:
    ...
    uploaded_files = st.file_uploader(
        "Carica uno o più file .pdf o .txt", type=["pdf", "txt"], accept_multiple_files=True
    )

    if uploaded_files:
        nuovi_file = [f for f in uploaded_files if f.name not in st.session_state.uploaded_file_names]
        if nuovi_file:
            st.session_state.pipeline.add_uploaded_files(nuovi_file)
            st.session_state.uploaded_file_names.update(f.name for f in nuovi_file)
